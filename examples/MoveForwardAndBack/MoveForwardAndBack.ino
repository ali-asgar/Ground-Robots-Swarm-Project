 ...
 *
 * Program Name: MoveForwardAndBack.ino
 ...
 *               
 * Purpose of this file: to turn the robot go straight and come back
 * Program 9
 * 
 */
...
void loop() {    
    ...
    if(enc1 > (radius + theta) && enc1 <= (radius + (1.5*theta)))  {
        roboclaw.BackwardM1(address,s1);
        roboclaw.ForwardM2(address,s2);
    }
    if(enc1 > (radius + (1.5*theta)) && enc1 <= ((2*radius) + (1.5*theta)))  {
       roboclaw.ForwardM1(address,s1);
       roboclaw.ForwardM2(address,s2);
    }
    if(enc1 > ((2*radius) + (1.5*theta)))  {
